...
applyButton.addEventListener('click', async () => {
    ...
    if (isEdit) {
        ...
        try {
            // створення за допомогою fetch і метода POST
            const result = await fetch('https://5e64c3daa49c210016106bc4.mockapi1.io/tasks', {
                method: 'POST',
...
const renderTodos = async (filter) => {
    // сторення промісу за допомогою fetch, і за допомогою then опарьовує результати виконання промісу
    ...
    // аналогічна дія за допомогою async/await
    let todos = [];
    try {
        const result = await fetch('https://5e64c3daa49c210016106bc4.mockapi.io/tasks');
